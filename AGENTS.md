...
- **oracle 不是纯语法检查器**：`zsh -n` 会求值一部分内容（除零、fd 号、进程替换），
  ...
  `bash -n` 的另一类盲区是**运行期才生效的解析选项**：`shopt -s extglob` 在 `-n`
  下不执行，所以 `echo @(?|.?)` 会被它拒而我们接受——这类要判成 oracle 少了上下文，
  不是我们的缺口
- **非 Linux 未验证**：只在 Linux 上跑过，macOS 的 shell 差异没有环境验证


## 「我们太宽松」那一格：2 个，全是 oracle 局限

- `extglob4.sub` / `extglob6.sub`：`echo @(?.|.?)` 这类模式在 `shopt -s extglob`
  下合法，而 `bash -n` 不执行 shopt 所以它拒。这是 oracle 的局限，实测
  `bash -O extglob -n` 与 `zsh -n` 都过。`run.sh` 里登记成 `oracle_blind`，
  输出会点名「其中 oracle 判不了的：2」，并对账：登记了却没出现、或出现了没登记，
  都会被打印出来，所以这张表不会悄悄替真缺口打掩护
- `exportfunc1.sub` 曾经属于这一格（一行 17 个 here-document）。它不是 oracle 局限，
  是真缺口，已修：`shell.h:172` 的 `HEREDOC_MAX 16`，第 17 个在 `push_heredoc`
  里报错退出；实测 16 个过、17 个不过、两行各 8 个也过（计数按行），zsh 无上限。
  现在这条消息进了证据表，文件也移到「两边都报错」格
...
`--help-id` 查询，结论是不值：真正需要调用方采取不同动作的警告只有一个，为它建一套查询
系统属于按预期建设。现在的约定是：警告文案有稳定前缀（`no --cwd given`），要程序化判定就
匹配前缀，别匹配整句。加警告仍然只有 `add_note` 一个入口。
