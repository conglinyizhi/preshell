...
- 基线 886/353 → 现在 **1192 通过 / 47 缺口 / 5 太宽松 / 3 两边报错 / 0 崩溃**。
  缺口逐刀下降：方言分派加 always 块（345）、词位置的括号归入词（284）、
  右花括号不需前置分隔符（264）、数字范围 glob（256）、展开里的花括号规则（255）、
  数组元素里的括号（181）、case 模式分组与多循环变量（50）、csh 风格循环（47）
