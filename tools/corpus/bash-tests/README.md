...
- 只取了 `*.sub`，没有取 `.tests` / `.right` / `run-*` 驱动（那是执行侧的东西，本项目不执行）
- 内容未做任何修改
- `run-minimal` 一并保留，它是上游用来跑最小语法集的驱动脚本，作为参考

## 如果将来要改本项目的许可

这份语料是 GPL-3.0-or-later 的第三方内容，随仓库分发会使整个分发物落在 GPL 之下。
本项目本来就选了 GPL-3.0-or-later，所以没有冲突；但如果将来想把项目改为宽松许可，
需要先删掉这个目录并把差分脚本改成从外部路径读语料（`run.sh` 的目录参数就是这个用途）。
