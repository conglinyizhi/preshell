...
`rm -rf dist` 报告的是 `Delete: dist`，不是绝对路径。这个工具不读磁盘，也不知道
你在哪个目录里调用它，所以**相对路径的起点必须由调用方给出**。

| 你怎么调用 | 报告里的路径 | `impact.cwd` |
|---|---|---|
| 不给基准，命令也不 `cd` | 原样相对：`dist` | 缺失 |
| 传 `--cwd=/a/b` | 绝对：`/a/b/dist` | `/a/b` |
| 命令自己 `cd /tmp` | 绝对：`/tmp/dist` | `/tmp` |

规则一句话：**能给绝对路径就给绝对路径，数据不够就退回原样，并把基准交代清楚。**

- `--cwd=PATH` 必须是绝对路径。相对值直接是用法错误（退出码 2）：相对基准没有可以
  参照的东西，接受它等于给出一份看着已解析、其实没有的路径
- `--cwd` 是解析起点，不是 `cd`：它不产生任何效果，也不出现在 `effects` 里
- 命令内部的 `cd` 优先于 `--cwd`
- `impact.cwd` 就是这份报告里相对路径的基准。缺失有两种含义：命令没有 `cd` 且你没给
  基准（此时相对路径相对你自己的 pwd），或者基准根本算不出来（此时 `uncertain` 是 true）

### 要求：调用方必须提供 pwd

**人或 agent 调用时，要么传 `--cwd`，要么自己完成拼接。** 不要看到相对路径就假定它
相对「当前目录」：报告里没有这个信息，按假定拼出来的路径可能指向别处，而报告是
`Complete`，从状态上完全看不出来。

做不到提供 `--cwd` 时，只有一条安全的用法：把报告里的相对路径**原样**交给在你的真实
工作目录里执行的那一侧，由它去解析。工具不替这一步做决定。
...
基准算不出来时（`cd $DIR`、未知基准下的相对 `cd`），报告置 `uncertain: true`，
不把「缺失」静默当成「没有变化」。
...
- `impact.cwd` — 这份报告里相对路径的基准，只有你传了 `--cwd` 或命令自己 `cd` 时才有值。
  缺失时的读法见上面「相对路径与 pwd」，别默认它相对你的当前目录
...
- [ ] 相对路径有归宿：要么传了 `--cwd`，要么把相对路径原样交给在真实目录里执行的那一侧
