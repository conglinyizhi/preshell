...
| 4 | `=(...)` 进程替换 | `zf_ln -fn =(<<<'') $tmp` | 待做。**注意：它会写临时文件**，effects 层要体现 |
| 5 | case 模式里的 `<1->` 数字范围 | `(<1->\|\*) amap[1]="$1";;` | 待做：模式上下文里 `<` 是词内容而不是重定向 |
| 6 | `${x//[^\\}]/}` 这类展开 | 括号表达式里的 `}` 不结束参数展开 | 待做，**bash 侧也错**（不是 zsh 专属） |
| 7 | 数组下标带标志 | `$argv[(I)--]`、`${a[1:3]}` | 待做 |
| 8 | `${...}` 的 zsh 修饰 | `${^x}`、`${(f)x}`、`${x:#pat}` | 待做 |
| 9 | csh 风格循环 | `foreach x (...) ... end`、`repeat n { }` | 待做 |
